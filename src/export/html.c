...
"#graph { display: block; width: 60vw; height: 80vh; min-height: 600px; margin: 20px 0; border: 1px solid #333; border-radius: 8px; background: #0f3460; overflow: hidden; box-sizing: border-box; }\n"
...
"  // Adapt force parameters to graph size\n"
"  const nodeCount = graphNodes.length;\n"
"  const linkDist = nodeCount > 100 ? 150 : nodeCount > 50 ? 120 : 80;\n"
"  const chargeStr = nodeCount > 100 ? -600 : nodeCount > 50 ? -400 : -200;\n"
"  const sim = d3.forceSimulation(graphNodes)\n"
"    .force('link', d3.forceLink(links).id(d=>d.id).distance(linkDist))\n"
"    .force('charge', d3.forceManyBody().strength(chargeStr))\n"
"    .force('center', d3.forceCenter(width/2, height/2))\n"
"    .force('collide', d3.forceCollide(nodeCount > 50 ? 30 : 15));\n"
...
"      .on('drag',(e,d)=>{d.fx=e.x;d.fy=e.y;}).on('end',(e,d)=>{if(!e.active)sim.alphaTarget(0);d.fx=d.x;d.fy=d.y;}));\n"
